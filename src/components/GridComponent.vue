  ...
  <div class="grid">
    <div v-for="(n, m) in 3" :key="n.id">
      <div v-for="(n, p) in 3" :key="n.id">
        <cell-component
          @clickOnCell="putSymbol(m, p)"
          :value="board[m][p]"
          :fontColor="fontColor[m][p]"
        ></cell-component>
      </div>
    </div>
    <div>
      <img
        v-show="this.whoseTurn == 'X'"
        class="XTurn"
        src="../assets/up-to-x.png"
        alt="Up to X"
      />
      <img
        v-show="this.whoseTurn == 'O'"
        class="OTurn"
        src="../assets/up-to-o.png"
        alt="Up to O"
      />
    </div>
  </div>
    ...
    checkGameSituation() {
      ...
      for (let i = 0; i < 3; i++) {
        if (this.board[i].includes('')) {
          // checks that at least in a row there's an empty cell
          this.stillPlaying = true;
        }
      }
